...
// FB_PAGE_ACCESS_TOKEN may be a Business **System User** token (non-expiring),
// which acts AS the system user — to post AS the Page we exchange it for the
// Page's own token. If the configured token is already a Page token, this call
// returns it unchanged; on any error we fall back to using it directly.
async function pageAccessToken(pageId, token) {
  try {
    const res = await fetch(`${GRAPH}/${pageId}?fields=access_token&access_token=${encodeURIComponent(token)}`);
    const json = await res.json().catch(() => ({}));
    if (res.ok && json && json.access_token) return json.access_token;
  } catch (e) {
...
export async function publishToFacebook({ message, mediaUrls, link } = {}) {
  ...
  const pageToken = await pageAccessToken(pageId, token); // works with a System User OR a Page token
  const photos = (mediaUrls || []).filter(Boolean);

  if (photos.length === 0) {
    const json = await graphPost(`${pageId}/feed`, { message, link, access_token: pageToken });
    return { id: json.id, url: permalink(json.id) };
  }

  if (photos.length === 1) {
    const json = await graphPost(`${pageId}/photos`, { url: photos[0], caption: message, access_token: pageToken });
  ...
  for (const u of photos.slice(0, 10)) {
    try {
      fbids.push(await uploadUnpublishedPhoto(pageId, pageToken, u));
    } catch (e) {
  ...
  const json = await graphPost(`${pageId}/feed`, { message, attached_media, access_token: pageToken });
  return { id: json.id, url: permalink(json.id) };
}
