...
export async function listAccountsAndLocations() {
  ...
  for (const acc of accounts) {
    // Use the v4 API to list locations: it returns each location's `name`
    // ("accounts/X/locations/Y") — exactly what localPosts needs. The newer
    // businessinformation list omits the resource name when a readMask is set.
    const locRes = await fetch(`${POSTS_API}/${acc.name}/locations?pageSize=100`, { headers: h });
    let locations;
    if (locRes.ok) {
      const data = await locRes.json();
      locations = (data.locations || []).map((l) => ({
        name: l.name,
        title: l.locationName || l.title || '(untitled)',
        address: l.address?.addressLines?.join(', ') || null,
      }));
    } else {
      locations = [{ _error: `v4 locations.list ${locRes.status}: ${(await locRes.text()).slice(0, 400)}` }];
    }
...
export async function publishLocalPost({ accountId, locationId, summary, mediaUrls, ctaType, ctaUrl }) {
  ...
  if (mediaUrls && mediaUrls.length) {
    body.media = mediaUrls.map((url) => ({ mediaFormat: 'PHOTO', sourceUrl: url }));
  }
