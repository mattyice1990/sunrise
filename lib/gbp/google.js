...
export async function listAccountsAndLocations() {
  ...
  for (const acc of accounts) {
    // readMask must NOT include 'name' (output-only) or Google returns 400.
    const readMask = 'title,storefrontAddress,phoneNumbers';
    ...
    if (locRes.ok) {
      ...
      // Surface the error instead of hiding it, so misconfig is visible.
      locations = [{ _error: `locations.list ${locRes.status}: ${(await locRes.text()).slice(0, 400)}` }];
    }
