...
/** Pick the best service page for a post (explicit serviceType or keyword scan). */
export function matchService(post) {
  const hay = `${norm(post.serviceType)} ${norm(post.note)} ${norm(post.finalCopy || post.draftCopy)}`;
  for (const m of GBP.serviceMappings) {
    if (post.serviceType && norm(post.serviceType) === norm(m.service)) return m;
    if (m.keywords.some((k) => hay.includes(k))) return m;
  }
  return { service: post.serviceType || 'Roofing', page: GBP.defaultServicePage };
}
