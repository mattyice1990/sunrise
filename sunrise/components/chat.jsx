...
function botReply(text) {
  ...
  if (/(leak|emergency|storm|monsoon|urgent)/.test(t))
...
function ChatWidget() {
  ...
  const [msgs, setMsgs] = useState([
    { from: "bot", text: "👋 Hi! Thanks for visiting Sunrise Roofers LLC. How can we help with your roof today?" },
  ]);
  ...
  return (
    <div className="chat">
      {open && (
        <div className="chat__panel" role="dialog" aria-label="Chat with Sunrise Roofers">
          <div className="chat__head">
            <span className="chat__ava"><Icon name="home" /></span>
            <div className="chat__who">
              <span className="chat__name">Sunrise Roofers</span>
              <span className="chat__status"><span className="chat__online"></span> Online · Replies in minutes</span>
            </div>
            <button className="chat__close" onClick={() => setOpen(false)} aria-label="Close chat"><Icon name="plus" /></button>
          </div>
          <div className="chat__body" ref={bodyRef}>
            {msgs.map((m, i) => (
              <div key={i} className={"chat__msg chat__msg--" + m.from}>{m.text}</div>
            ))}
            {typing && <div className="chat__msg chat__msg--bot chat__typing"><span></span><span></span><span></span></div>}
            {msgs.length <= 1 && !typing && (
              <div className="chat__quick">
                {CHAT_QUICK.map((q) => (
                  <button key={q} className="chat__chip" onClick={() => send(q)}>{q}</button>
                ))}
              </div>
            )}
          </div>
          ...
          <div className="chat__foot">Or call us 24/7 · <a href="tel:5207531758">520-753-1758</a></div>
        </div>
      )}

      {!open && greet && (
        <div className="chat__greet">
          <button className="chat__greet-x" onClick={() => setGreet(false)} aria-label="Dismiss">×</button>
          <b>Questions about your roof?</b>
          <span>Message us — we reply fast.</span>
        </div>
      )}

      <button className={"chat__launch" + (open ? " is-open" : "")} onClick={() => { setOpen(!open); setGreet(false); }} aria-label="Open chat">
        {open ? <Icon name="plus" /> : <Icon name="chat" />}
        {!open && <span className="chat__dot"></span>}
      </button>
    </div>
  );
}
