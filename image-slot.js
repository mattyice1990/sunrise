...
(() => {
  ...
  const stylesheet =
    ...
    // touch-action:pan-y lets a finger that lands on the image still scroll
    // the page vertically (the slot is a static photo on the live site). Only
    // in reframe mode (below) do we lock it to none so pan/zoom gestures work.
  ...
  class ImageSlot extends HTMLElement {
    ...
    _render() {
      ...
      // Alt text for SEO/accessibility — falls back to the placeholder label.
      this._img.alt = this.getAttribute('alt') || this.getAttribute('placeholder') || '';
      this._ghost.alt = this._img.alt;
