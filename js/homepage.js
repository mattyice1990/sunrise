// Homepage-specific JavaScript (optimized, unused code removed)
...
const statsObserver = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
        if (entry.isIntersecting) {
            ...
            if (mileCounter && !mileCounter.dataset.animated) {
                mileCounter.dataset.animated = 'true';
                animateCounter(mileCounter, 40);
            }
...
const statsSection = document.querySelector('#mile-radius-counter');
if (statsSection) {
    statsObserver.observe(statsSection.closest('section') || statsSection);
}

// ============================================
// TOGGLE SERVICES GRID
// ============================================
function toggleServices() {
    const servicesGrid = document.getElementById('servicesGrid');
    const toggleBtn = document.getElementById('toggleServicesBtn');
    
    if (servicesGrid && toggleBtn) {
        servicesGrid.classList.toggle('expanded');
        const isExpanded = servicesGrid.classList.contains('expanded');
        toggleBtn.setAttribute('aria-expanded', isExpanded);
        toggleBtn.innerHTML = isExpanded 
            ? 'Show Less <span style="display: inline-block; transition: transform 0.3s; transform: rotate(180deg);">▼</span>' 
            : 'View All Services <span style="display: inline-block; transition: transform 0.3s;">▼</span>';
    }
}
